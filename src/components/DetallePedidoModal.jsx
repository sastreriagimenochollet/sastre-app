...
import { PROVEEDORES_CAMISERIA, TEJIDO_FABRICA_CAMISERIA, ESTADOS_CAMISERIA } from '../lib/camiseriaConfig'
...
export default function DetallePedidoModal({ pedido, onCerrar }) {
  ...
  useEffect(() => {
    ...
    cargarPrendas()
    cargarTejidos()
    if (esCamiseria) cargarCamiseria()
  }, [pedido.id])
  ...
  async function cargarCamiseria() {
    const { data } = await supabase
      ...
      .maybeSingle()
    if (data) {
      setCamSeguimiento(data)
      setCamProveedor(data.proveedor ?? '')
      setCamReferencia(data.referencia ?? '')
      setCamTejido(data.tejido_fabrica ?? '')
    }
  }

  async function guardarCamiseria(cambios) {
    const { data, error } = await supabase
      .from('camiseria_seguimiento')
      .upsert({ pedido_id: pedido.id, ...cambios }, { onConflict: 'pedido_id' })
      .select()
      .single()
    if (!error && data) setCamSeguimiento(data)
  }
      ...
      <div className="bg-lino w-full sm:max-w-4xl sm:rounded-card rounded-t-2xl max-h-[92vh] overflow-y-auto scroll-suave">
        ...
        <div className="p-5 space-y-5">
          ...
          {esCamiseria ? (
            ...
            <div>
              <p className="text-xs uppercase tracking-wide text-tinta/40 font-medium mb-1.5">Camisería</p>
              <div className="grid grid-cols-3 gap-2">
                <select
                  value={camProveedor}
                  onChange={(e) => { setCamProveedor(e.target.value); guardarCamiseria({ proveedor: e.target.value || null }) }}
                  className="bg-white border border-bronce-100 rounded-lg px-2.5 py-2 text-sm outline-none"
                >
                ...
                <input
                  ...
                  onBlur={(e) => guardarCamiseria({ referencia: e.target.value.trim() || null })}
                  placeholder="Referencia de tejido"
                  className="bg-white border border-bronce-100 rounded-lg px-2.5 py-2 text-sm outline-none"
                />
                <select
                  value={camTejido}
                  onChange={(e) => { setCamTejido(e.target.value); guardarCamiseria({ tejido_fabrica: e.target.value || null }) }}
                  className="bg-white border border-bronce-100 rounded-lg px-2.5 py-2 text-sm outline-none"
                >
