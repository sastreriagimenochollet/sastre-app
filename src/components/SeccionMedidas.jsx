...
import { ACENTO_SASTRERIA } from '../lib/constantes'
import {
  CAMPOS_AMERICANA,
  FORMULAS_AMERICANA,
  CAMPOS_PANTALON,
...
export default function SeccionMedidas({ medidas, guardarMedidas }) {
  ...
  useEffect(() => {
    // Solo tomamos los valores de "medidas" la primera vez que
    // llegan del servidor. Los refrescos posteriores (tras guardar)
    // no deben pisar lo que el usuario ya tiene en pantalla, porque
    // eso es lo que causaba el parpadeo al cambiar de campo.
    if (medidas && !cargadoInicial.current) {
      setValores(medidas)
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {SECCIONES.map((s) => (
          <button
            ...
            style={seccion === s.id ? { backgroundColor: ACENTO_SASTRERIA } : undefined}
          >
            {s.label}
          </button>
        ))}
              ...
              return (
                <div key={f.key} className="flex items-center justify-between px-4 py-2.5">
                  ...
                  <p
                    ...
                    style={{ color: resultado === null ? '#8A8378' : ACENTO_SASTRERIA }}
                  >
