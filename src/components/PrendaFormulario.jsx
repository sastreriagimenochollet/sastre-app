...
import { TIPOS_PRENDA, camposPorTipo, labelTipoPrenda, mapearMedidasCliente, mapearMedidasHaciaCliente } from '../lib/prendasConfig'
...
// medidas ya guardadas en la ficha del cliente, o guardar las de
// aquí hacia la ficha.
export default function PrendaFormulario({ prenda, onCambiar, onEliminar, colapsableInicial = false, clienteId, acento = '#7C9473' }) {
  ...
  async function guardarEnFicha() {
    ...
    const cambios = mapearMedidasHaciaCliente(prenda.tipo, prenda.detalles)
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {campos.confeccion.map((c) => {
                const valorActual = prenda.detalles?.[c.key]
                const mostrarSubcampo = c.campoCondicional && valorActual === c.campoCondicional.valorActivador
                return (
                  <Fragment key={c.key}>
                    <CampoDetalle campo={c} valor={valorActual} onCambiar={actualizarDetalle} />
                    {mostrarSubcampo && (
                      <CampoDetalle
                        campo={c.campoCondicional.subcampo}
                        valor={prenda.detalles?.[c.campoCondicional.subcampo.key]}
                        onCambiar={actualizarDetalle}
                      />
                    )}
                  </Fragment>
                )
