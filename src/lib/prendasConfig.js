...
export const CAMPOS_CHAQUETA = {
  ...
  confeccion: [
    ...
    { key: 'aberturas', label: 'Aberturas', tipo: 'select', opciones: ['0', '1', '2'] },
    { key: 'botones', label: 'Botones', tipo: 'select', opciones: ['1', '2', '3', '2/3', '4/1', '4/2', '6/2', '6/1'] },
    { key: 'bolsillos', label: 'Bolsillos', tipo: 'select', opciones: ['2', '3'] },
...
export const CAMPOS_PANTALON = {
  ...
  confeccion: [
    ...
    { key: 'trincha', label: 'Trincha (cm)', tipo: 'texto' },
    {
      ...
      label: 'Pasadores',
      tipo: 'select',
      opciones: ['7', '8', 'Ceñidores laterales', 'Ceñidor central', '8+Ceñidor'],
    },
    { key: 'bolsillo', label: 'Bolsillo', tipo: 'select', opciones: ['Italiano', '2 vivos', 'Costura', 'Otro'] },
    {
      key: 'revolvera',
      label: 'Revolvera',
      tipo: 'select',
      ...
      campoCondicional: {
        valorActivador: 'Sí',
        subcampo: { key: 'revolvera_cantidad', label: 'Cantidad', tipo: 'select', opciones: ['1', '2'] },
      },
    },
    {
      key: 'relojera',
      ...
      campoCondicional: {
        valorActivador: 'Sí',
        subcampo: { key: 'relojera_cantidad', label: 'Cantidad', tipo: 'select', opciones: ['1', '2'] },
      },
    },
    {
      key: 'delantero',
      label: 'Delantero',
      tipo: 'select',
      opciones: ['Gafa Tira Botón', 'Acabado Recto', 'Cinturilla Extendida'],
    },
    { key: 'ardillon', label: 'Ardillón', tipo: 'select', opciones: ['Sí', 'No'] },
    {
      ...
      label: 'Tipo de bajo',
      tipo: 'select',
      opciones: ['Recto', 'Vuelta'],
      campoCondicional: {
        valorActivador: 'Vuelta',
        subcampo: { key: 'bajo_vuelta_cm', label: 'CM vuelta', tipo: 'select', opciones: ['3', '3,5', '4', '4,5', '5'] },
      },
    },
...
// Dirección inversa: toma las medidas escritas en una prenda del
// pedido y arma el objeto a guardar en la ficha del cliente
// (botón "Guardar en ficha").
export function mapearMedidasHaciaCliente(tipo, detalles) {
  const mapeo = tipo === 'chaqueta' ? MAPEO_CHAQUETA : tipo === 'pantalon' ? MAPEO_PANTALON : MAPEO_CHALECO
  const resultado = {}
  for (const [campoLocal, campoCliente] of Object.entries(mapeo)) {
    const valor = detalles?.[campoLocal]
    if (valor !== null && valor !== undefined && valor !== '') resultado[campoCliente] = valor
  }
  return resultado
}
