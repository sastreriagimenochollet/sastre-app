...
export const RUBROS = [
  { id: 'sastreria', label: 'Sastrería', color: '#7C9473' },
...
export function colorRubro(id) {
  return RUBROS.find((r) => r.id === id)?.color ?? '#7C9473'
}

// Calcula cuán urgente es un pedido según su fecha de entrega.
// Devuelve null si no aplica (sin fecha, o ya entregado).
export function urgenciaEntrega(fechaEntrega, yaEntregado) {
  if (!fechaEntrega || yaEntregado) return null
  const hoy = new Date()
...
// Acento visual por sección: sastrería usa blanco + verde salvia;
// camisería usa el azul de su etapa "Pasada". Nada de dorado.
export const ACENTO_SASTRERIA = '#7C9473'
export const ACENTO_CAMISERIA = '#5B7C99'

export function acentoDeRubro(rubro) {
  return rubro === 'camiseria' ? ACENTO_CAMISERIA : ACENTO_SASTRERIA
}
