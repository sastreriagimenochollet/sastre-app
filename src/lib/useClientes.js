...
export function useCliente(id) {
  ...
  async function guardarMedidas(cambios) {
    // Actualización optimista: refleja el cambio al instante en
    // pantalla, sin esperar ni recargar todo, así no hay parpadeo.
    setMedidas((prev) => ({ ...(prev ?? {}), ...cambios }))
    ...
    if (medidas) {
      ...
      const { data, error } = await supabase
        .from('medidas')
        .insert({ cliente_id: id, ...cambios })
        .select()
        .single()
      if (error) throw error
      setMedidas(data)
    }
    // Refresco silencioso en segundo plano, sin activar el estado
    // "cargando" (que es lo que causaba el parpadeo).
    cargar(false)
  }
